...
<head>
    ...
    <style>
        ...
        .modal-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.5); backdrop-filter: blur(4px); z-index: 100; display: flex; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: 0.3s; }
...
<body class="pb-32">
    ...
    <div id="modal-history" class="modal-overlay p-4" onclick="if(event.target === this) closeHistory()">
        <div class="modal-content overflow-hidden flex flex-col max-h-[90vh]">
            <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                <div>
                    <h3 id="modal-title" class="text-xl font-black text-slate-900 uppercase italic">Suivi Satisfaction</h3>
                    <p id="modal-subtitle" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Détails et historique des sollicitations</p>
                </div>
            ...
            <div class="p-8 overflow-y-auto flex-1 space-y-8">
                ...
                <div class="space-y-4">
                    <h4 class="text-[10px] font-black uppercase text-slate-400 italic tracking-widest flex items-center gap-2">
                        <i class="fa-solid fa-history"></i> Historique des envois
                    </h4>
            ...
            <div class="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                <button id="modal-resend-btn" class="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition flex items-center justify-center gap-2">
                    <i class="fa-solid fa-sync-alt"></i> Renvoyer le questionnaire
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let tokenMap = <?= json_encode($tokens) ?>;
        let responseMap = [];
                ...
                listContainer.innerHTML = recipients.map(r => {
                    ...
                    let statusHtml = `
                        <div class="flex gap-1 shrink-0">
                            <button onclick="openSatisfactionHistory('${token}', '${r.email}', '${r.firstName.replace(/'/g, "\\'")}', '${r.lastName.replace(/'/g, "\\'")}', '${r.orderId}', '${r.itemName.replace(/'/g, "\\'")}')" class="w-6 h-6 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-blue-600 hover:text-white transition" title="Détails du suivi">
        ...
        async function openSatisfactionHistory(token, email, firstName, lastName, orderId, itemName) {
                        ...
                        attemptsList.innerHTML = data.attempts.map(a => `
                            <div class="p-4 rounded-2xl border ${a.status === 'success' ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}">
                                <div class="flex justify-between items-start mb-1">
                                    ...
                                    <span class="text-[9px] font-bold text-slate-400">${new Date(a.created_at).toLocaleString('fr-FR')}</span>
                                </div>
                                ${a.error_message ? `<p class="text-[10px] font-medium text-red-400 mt-2 bg-white/50 p-2 rounded-lg border border-red-50">${a.error_message}</p>` : ''}
                            </div>
                        `).join('');
