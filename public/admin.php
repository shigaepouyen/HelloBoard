...
<body class="pb-32">
    ...
    async function editCamp(org, slug, type, name) {
        ...
        (data.apiItems || []).forEach(item => { if(!rules.find(r => r.pattern === item.pattern)) rules.push({ pattern: item.pattern, displayLabel: item.pattern, type: item.isMain ? 'Billet' : 'Option', group: 'Divers', chartType: 'pie', transform: '', hidden: false, costPrice: 0, sellingPrice: 0 }); });
        const headerHtml = `
            <div class="hidden lg:flex items-center gap-4 px-6 mb-2 text-[10px] font-black uppercase text-slate-400 italic">
                <div class="w-12 text-center">Ordre / Actif</div>
                <div class="flex-1 ml-4">Nom de l'article (Source HelloAsso)</div>
                <div class="w-32">Nom du Bloc</div>
        ...
        zone.innerHTML = `
            <div class="mt-20 pt-20 border-t border-slate-200">
                <div class="flex justify-between items-center mb-12">
                    ...
                    <button id="save-main-btn" class="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs shadow-xl shadow-blue-200">Sauvegarder</button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div class="admin-card p-8">
                        ...
                        <div class="grid gap-4">
                            <div>
                                <label class="text-[10px] font-bold text-slate-500 uppercase block mb-1">Cible Financière (€)</label>
                                <input type="number" id="goal-rev" class="input-soft" value="${goals.revenue}">
                            </div>
                            <div>
                                <label class="text-[10px] font-bold text-slate-500 uppercase block mb-1">${labels.quota} (Qté)</label>
                                <input type="number" id="goal-tix" class="input-soft" value="${goals.tickets}">
                            </div>
                            <div>
                                <label class="text-[10px] font-bold text-slate-500 uppercase block mb-1">Ref N-1</label>
                    ...
                    <div class="admin-card p-8">
                        <h3 class="text-xs font-black uppercase text-slate-400 mb-6 italic">Marqueurs Timeline</h3>
                        ...
                        <button onclick="addMarkerRow()" class="text-[10px] font-black text-blue-600 uppercase hover:underline">+ Nouveau marqueur</button>
                    </div>
                </div>

                <div class="space-y-4">
                    <div class="flex items-center justify-between px-2">
                        <h3 class="text-xs font-black uppercase text-slate-400 italic">Configuration de l'importation</h3>
                        <span class="text-[10px] font-bold text-slate-300">Glissez-déposez pour réorganiser</span>
                    </div>
                    ...
                    <div id="rules-list" class="space-y-2">
                        ${rules.map(r => `
                            <div class="rule-tile admin-card p-5 flex flex-col lg:flex-row items-center gap-4 group hover:border-blue-200 transition-colors" data-item="${r.pattern}">
                                <div class="flex items-center gap-4 w-full lg:w-auto">
                                    <div class="cursor-grab text-slate-300 group-hover:text-blue-300"><i class="fa-solid fa-grip-lines"></i></div>
                                    <div class="toggle-btn ${r.hidden ? '' : 'active'}" onclick="this.classList.toggle('active')"></div>
                                </div>

                                <div class="flex-1 w-full min-w-0">
                                    <input type="text" class="display-label input-soft !py-2 !text-sm border-transparent focus:border-blue-500" value="${r.displayLabel}">
                                    <p class="text-[9px] font-bold text-slate-300 uppercase mt-1 truncate italic pl-1">Source : ${r.pattern}</p>
                                </div>

                                <div class="grid grid-cols-2 lg:flex lg:items-center gap-2 w-full lg:w-auto">
                                    <div class="lg:w-32">
                                    ...
                                    <div class="lg:w-24">
                                        <select class="rule-chart input-soft !py-2 !px-3 !text-[10px] uppercase font-black">
                                            <option value="pie" ${r.chartType==='pie'?'selected':''}>Camembert</option>
                                            <option value="bar" ${r.chartType==='bar'?'selected':''}>Barres</option>
                                        </select>
                                    </div>
                                    <div class="lg:w-24">
                                        <input type="text" class="rule-transform input-soft !py-2 !px-3 !text-[10px] font-mono text-blue-500" value="${r.transform || ''}" placeholder="REGEX" title="Transformation avancée">
                                    </div>
